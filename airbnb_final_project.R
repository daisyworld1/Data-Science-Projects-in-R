...
train6 <- subset(train6,country_destination != "NDF" | country_destination != "US")
...
#SmoteClassif
library(UBL)
train_sci <- data.frame(as.numeric(train$age), as.factor(train$gender),as.factor(train$language_full),train$country_destination)
colnames(train_sci) <- c("age", "gender", "language", "country_destination")
train_sci <- subset(train_sci, age > 18 & age <= 65)
train_sci <- na.omit(train_sci)

table(train_sci$country_destination)

new_train_sci <- SmoteClassif(country_destination ~ ., train_sci, C.perc =  "balance", dist = "HEOM")

table(new_train_sci$country_destination)

#redo ramdom forest model after balancing the dataset
new_train_sci$age_cat <- as.numeric(new_train_sci$age)
new_train_sci$age_cat[is.na(new_train_sci$age_cat)] <- -1
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >1000, 2015 - new_train_sci$age_cat, new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >65 & new_train_sci$age_cat < 1000,"above 65", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >55 & new_train_sci$age_cat <= 65 ,"between 56 and 65", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >45 & new_train_sci$age_cat <= 55 ,"between 46 and 55", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >35 & new_train_sci$age_cat <= 45 ,"between 36 and 45", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >25 & new_train_sci$age_cat <= 35 ,"between 26 and 35", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat >18 & new_train_sci$age_cat <= 25 ,"between 18 and 25", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat > 0 & new_train_sci$age_cat <= 18,"under 18", new_train_sci$age_cat)


new_train_sci$age_cat <- as.character(new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat == 7, "under 18", new_train_sci$age_cat)
new_train_sci$age_cat <- ifelse(new_train_sci$age_cat == -1, "-unknown-", new_train_sci$age_cat)

set.seed(3000)

new_train_sci <- na.omit(new_train_sci)

split <- sample.split(new_train_sci$country_destination, SplitRatio = 0.7)
new_train_sci_test <- subset(new_train_sci, split == FALSE)
new_train_sci_train <- subset(new_train_sci, split == TRUE)
trainTree2 <- rpart(country_destination ~ gender + age_cat + language, data=new_train_sci_train, method="class", control=rpart.control(minbucket=1000))
prp(trainTree2)
PredictCART2 <- predict(trainTree2, newdata=new_train_sci_test, type="class")
table(new_train_sci_test$country_destination,PredictCART2)

trainForest3 <- randomForest(country_destination ~ gender + age + language, data=new_train_sci, nodesize=25, ntree=200)
PredictForest3 <- predict(trainForest3, newdata=new_train_sci_test)
table(new_train_sci_test$country_destination, PredictForest3)

fitControl <- trainControl(method="cv", number=10)
cartGrid <- expand.grid(.cp=(1:50)*0.01)
train(country_destination ~ gender + age_cat + language_full + device_type + browser_type, data=new_train_sci_train, method="rpart", trControl=fitControl, tuneGrid=cartGrid)
trainTreeCV2 <- rpart(country_destination ~ gender + age_cat + language, method="class", data=new_train_sci_train, control=rpart.control(cp=0.18))
predictCV2 <- predict(trainTreeCV2, newdata=new_train_sci_test, type="class")
table(new_train_sci_test$country_destination, predictCV2)
